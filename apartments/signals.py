...
from utils.image_manipulation import reduce_quality, resize_image, create_thumbnail
...
def reduce_apartment_image_quality_and_optimize(sender, instance, **kwargs):
    ...
    reduce_quality(instance.image, instance.image.name,
                   new_width=1920, quality=70)


@receiver(post_save, sender=ApartmentImage)
def create_thumbnail_for_first_image(sender, instance, **kwargs):
    if instance == instance.apartment.images.first():
        create_thumbnail(instance.image, instance.image.name,
                         new_width=400, new_height=400)
